...
public class ChessMatch {
    ...
    public ChessPiece performChessMovie(ChessPosition sourcePosition, ChessPosition targetPosition) {
        ...
        // verificando se a peça movida foi um peão e se foi um movimento inicial de peão com duas casas
    ...
    private ChessPiece newPiece(String type, Color color) {
        ...
        return new Rook(board, color); // não preciso de if aqui
        // pq se falar em cima então vai ser o rook
    }

    private Piece makeMove(Position source, Position target) {
        // faço um downcast
        ChessPiece p = (ChessPiece) board.removePiece(source); // removendo peça da posição de origem
        p.increaseMoveCount();
        Piece capturedPiece = board.removePiece(target); // removendo possivel peça na posição de destino
        //faz um upcasting naturalmente
        ...
        if (p instanceof King && target.getColumn() == source.getColumn() - 2) {
            ...
            Position targetT = new Position(source.getRow(), source.getColumn() - 1);
            ChessPiece rook = (ChessPiece) board.removePiece(sourceT); // tirando a torre da pos de origem
            board.placePiece(rook, targetT); // colocando a torre na pos de destino
            rook.increaseMoveCount();
        }
    ...
    private void undoMove(Position source, Position target, Piece capturedPiece) {
        ...
        if (p instanceof King && target.getColumn() == source.getColumn() - 2) {
            Position sourceT = new Position(source.getRow(), source.getColumn() - 4);
            Position targetT = new Position(source.getRow(), source.getColumn() - 1);
            ChessPiece rook = (ChessPiece) board.removePiece(targetT); // tirando da posição de destino
            board.placePiece(rook, sourceT); // colocando na posição de origem
            rook.decreaseMoveCount();
        }
